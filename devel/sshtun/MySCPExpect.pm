...
sub _set{
   my($self,$attr,$val) = @_;
   die("No attribute supplied to 'set()' method") unless defined $attr;
   $self->{"_$attr"} = $val;
}

sub auto_yes{
   my($self,$val) = @_;
   die("No value passed to 'auto_yes()' method") unless defined $val;
   $self->_set('auto_yes',$val);
}

sub error_handler{
   my($self,$sub) = @_;
   die("No sub supplied to 'error_handler()' method") unless defined $sub;
...
sub login{
   ...
   die("No user supplied to 'login()' method") unless defined $user;
   die("No password supplied to 'login()' method") if @_ > 2 && !defined $password;
...
sub password{
   my($self,$password) = @_;
   die("No password supplied to 'password()' method");
...
sub host{
   my($self,$host) = @_;
   die("No host supplied to 'host()' method") unless $host;
...
sub user{
   my($self,$user) = @_;
   die("No user supplied to 'user()' method") unless $user;
   $self->_set('user',$user);
} 
...
sub scp{
   ...
   die("No login. Can't scp") unless $login;
   #die("No password or identity file. Can't scp") unless $password || $identity_file;
   die("No host specified. Can't scp") unless $host;
   ...
   print STDERR "[Debug] \$scp_string='$scp_string'\n";
   $scp = Expect->spawn($scp_string);
   
   unless ($scp) {
      if($handler){ $handler->($!); return; }
      else { die("Couldn't start program: $!"); }
   }
   ...
   if ($password) {
      unless($scp->expect($timeout,-re=>'[Pp]assword.*?:|[Pp]assphrase.*?:')){
         ...
         if($err){
            ...
            else { die("Problem performing scp: $err"); }
         }
...
#          else{ die($err) };
      ...
      $error = ($scp->expect($timeout_err,
         ...
         [qr/\w+.*/ => sub{
               ...
               if($handler){
                  ...
                  die("Error: last line returned was: $error");
               }
            }
         ],
         ['eof' => sub{ $eof = 1 } ],
   ...
   if ($error && !($eof && $error =~ m/^(2|3)/o)) {
      if ($handler) {
         ...
         die("scp processing error occured: $error");
      }
   }
   ...
   if ($scp->exitstatus > 0) {   #ignore -1, in case there's a waitpid portability issue
      if ($handler) {
         ...
         die("scp exited with non-success state: " . $scp->exitstatus);
      }
   }
...
do not define an error handler, then a simple 'die()' call will occur, with the last
