...
my $time = time();
my $len = 0;
$len += $ch->sw->send_message({uuid => $_.'.1-'.$time, "data" => $_}, '#' x (1024 * 1024)) for 1..$max;
        $ch->sw->wait_for_ack(         $_.'.1-'.$time) for 1..$max;

my $time_d = time - $time;
my $bytes_sec = $len / $time_d;
...
trace "$0: Done\n";
trace "$0: Sent ".sprintf("%.02K", $len / 1024)." in ".sprintf("%.02s", $time_d)." at a rate of ".sprintf("%.02Kb/sec", $bytes_sec / 1024)."\n";
