...
#include "/opt/livepro/gfxengine/VideoThread.h"
#include "/opt/livepro/gfxengine/CameraThread.h"
#include "/opt/livepro/gfxengine/MjpegThread.h"
...
// 		texOrig = texOrig.convertToFormat(QImage::Format_RGB32);
	...
	while (doRendering) 
	{
		...
		if(newFrame) // && (time.elapsed() - lastFrameTime) >= frame.holdTime)
		{
			...
			texGL = QGLWidget::convertToGLFormat( frame->toImage() );
...
// 			glTexCoord2f(0.0f, 1.0f); glVertex3f(-1.0f,  1.0f, -1.0f);
			...
			double x0=0.,
			       ...
			       y2=1.;
			// Front Face
			glTexCoord2f(x0  , y0  ); glVertex3f(-1.0f, -1.0f,  1.0f);
			glTexCoord2f(x1  , y0  ); glVertex3f( 1.0f, -1.0f,  1.0f);
			glTexCoord2f(x1  , y1  ); glVertex3f( 1.0f,  1.0f,  1.0f);
			glTexCoord2f(x0  , y1  ); glVertex3f(-1.0f,  1.0f,  1.0f);
			// Back Face
			glTexCoord2f(x2  , y0  ); glVertex3f(-1.0f, -1.0f, -1.0f);
			...
			glTexCoord2f(x2  , y1  ); glVertex3f(-1.0f,  1.0f, -1.0f);
			glTexCoord2f(x2  , y0  ); glVertex3f(-1.0f,  1.0f,  1.0f);
			glTexCoord2f(x3  , y0  ); glVertex3f( 1.0f,  1.0f,  1.0f);
			glTexCoord2f(x3  , y1  ); glVertex3f( 1.0f,  1.0f, -1.0f);
			// Bottom Face
			glTexCoord2f(x1  , y2  ); glVertex3f(-1.0f, -1.0f, -1.0f);
			glTexCoord2f(x0  , y2  ); glVertex3f( 1.0f, -1.0f, -1.0f);
			glTexCoord2f(x0  , y1  ); glVertex3f( 1.0f, -1.0f,  1.0f);
			glTexCoord2f(x1  , y1  ); glVertex3f(-1.0f, -1.0f,  1.0f);
			// Right face
			glTexCoord2f(x2  , y1  ); glVertex3f( 1.0f, -1.0f, -1.0f);
			...
			glTexCoord2f(x2  , y1  ); glVertex3f(-1.0f, -1.0f, -1.0f);
			glTexCoord2f(x3  , y1  ); glVertex3f(-1.0f, -1.0f,  1.0f);
			glTexCoord2f(x3  , y2  ); glVertex3f(-1.0f,  1.0f,  1.0f);
			glTexCoord2f(x2  , y2  ); glVertex3f(-1.0f,  1.0f, -1.0f);
		glEnd();
