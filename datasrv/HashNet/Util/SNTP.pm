...
=head1 SNTP - Get time from an NTP time server
 ...
 Revision history: 
   ...
   V0.3   2012-09-27, Updated to work as a module in the HashNet namespace - Josiah Bryan

 Usage: HashNet::Util::SNTP->sync_time([$server]) - Defaults to 0.us.pool.ntp.org
...
# 	(!$ARGV[0]) && usage;     # no server: print usage message and exit
	...
	sub sync_time
	{
		...
		$server = shift || '0.us.pool.ntp.org';
		
		my $get_offset_only = shift || 0;
